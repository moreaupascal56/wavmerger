...
from wavmerger import concat_wav_in_dir, WavFile
...
def get_wav_test_data() -> tuple[Path,Path]:
    ...
    return (
        dirpath.joinpath("PinkPanther30.wav"),
        dirpath.joinpath("splitted")
    )




@pytest.mark.filterwarnings("ignore") # A warning is raised because of the way the test data is created, let's ignore it
def test_concat_wav_in_dir(
    get_wav_test_data, temp_dir
):
